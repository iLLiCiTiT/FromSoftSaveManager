...
public:
    ...
    static GameSaveTabButton* fromGame(const fssm::Game game, const QString& saveId, QWidget* parent) {
        ButtonGameInfo gi = getGameInfo(game);
        return new GameSaveTabButton(saveId, gi.icon, gi.title, parent);
    };
private:
    QString m_saveId = "";
    static ButtonGameInfo getGameInfo(fssm::Game game);
...
public:
    ...
    void addTab(const fssm::Game& game, const QString& saveId);
