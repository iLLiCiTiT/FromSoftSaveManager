...
class CharsListModel(QtGui.QStandardItemModel):
    ...
    def __init__(self, controller, save_id, parent):
        ...
        self._chars_by_id: dict[int, EldenRingCharacter | None] = {
            idx : None
            for idx in range(10)
        }

    def refresh(self):
        chars_info = self._controller.get_er_characters(self._save_id)
    ...
    def get_char_by_id(self, item_id: int) -> EldenRingCharacter | None:
        return self._chars_by_id.get(item_id)


class ERWidget(QtWidgets.QWidget):
    _bg_pix = None
    _bg_overlay_pix = None
    ...
    def paintEvent(self, event):
        ...
        overlay_pix = self._get_bg_overlay_pix()
        ratio = float(rect.width()) / rect.height()
        src_width = pix.width()
        src_height = pix.height()
        if ratio > 1.0:
            src_width *= ratio
        else:
            src_width /= ratio
        ...
        painter.drawPixmap(rect, overlay_pix)

    @classmethod
    def _get_bg_pix(cls):
        if cls._bg_pix is None:
            cls._bg_pix = QtGui.QPixmap(
                get_resource("menu", "bg.png")
            )
        return cls._bg_pix

    @classmethod
    def _get_bg_overlay_pix(cls):
        if cls._bg_overlay_pix is None:
            cls._bg_overlay_pix = QtGui.QPixmap(
                get_resource("menu", "bg_overlay.png")
            )
        return cls._bg_overlay_pix
